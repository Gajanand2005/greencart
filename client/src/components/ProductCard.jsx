...
const ProductCard = ({ product }) => {
    const { cartItems, currency, addToCart, removeFromCart } = useAppContext();
        ...
        <div onClick={()=> {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white dark:bg-gray-800 min-w-50 max-w-50 w-full ">
            ...
            <div className="text-gray-500/60 dark:text-gray-400 text-sm">
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div className="text-indigo-500" onClick={(e) => {
                        e.stopPropagation();
                    }}>
                        {!cartItems[product._id] ? (
                            <button className="flex items-center justify-center gap-1 bg-indigo-400 hover:bg-purple-300 border border-indigo-300 md:w-[80px] w-[64px] h-[34px] rounded text-indigo-600 font-medium" onClick={() => addToCart(product._id)} >
                                <img src={assets.cart_icon} alt="cart_icon" className='brightness-200'/>
                                Add
                            </button>
                        ) : (
